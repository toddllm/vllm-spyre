...
def _semantic_check_from_summary(
    baseline_summary: dict[str, Any],
    candidate_result: dict[str, Any],
) -> dict[str, Any]:
    baseline_result = baseline_summary.get("result", {})
    baseline_ids = [int(token_id) for token_id in baseline_result.get("output_token_ids", [])]
    candidate_ids = [int(token_id) for token_id in candidate_result.get("output_token_ids", [])]
    if baseline_ids or candidate_ids:
        return {
            "match": baseline_ids == candidate_ids,
            "compare_by": "token_ids",
            "baseline_label": str(baseline_result.get("label", "")),
            "candidate_label": str(candidate_result.get("label", "")),
        }

    return {
        "match": str(baseline_result.get("output_text", "")) == str(candidate_result.get("output_text", "")),
        "compare_by": "text",
        "baseline_label": str(baseline_result.get("label", "")),
        "candidate_label": str(candidate_result.get("label", "")),
    }
...
def main() -> int:
    ...
    parser.add_argument("--role", choices=("prefill", "decode", "baseline"), required=True)
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional JSON file describing a real prompt pair to drive the sequential handoff demo.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to a prior baseline JSON artifact for semantic comparison.",
    )
    parser.add_argument(
        "--write-json",
        type=str,
        default=None,
        help="Optional path to write the final JSON summary artifact directly.",
    )
    ...
    if args.prompt_pair_json:
        ...
        effective_max_num_batched_tokens = args.max_num_batched_tokens
    else:
    ...
    try:
        ...
        if max(len(prompt_prefill_tokens), len(prompt_partial_tokens)) > effective_max_num_batched_tokens:
            raise SystemExit(
                "Prompt pair exceeds --max-num-batched-tokens. "
                "Increase the flag for this workload-derived prompt."
            )
        ...
        if args.role == "prefill":
            ...
            result = _run_once(
                llm,
                TokensPrompt(prompt_token_ids=baseline_prompt_tokens),
                sampling_params,
                f"{args.decode_variant}_cold_baseline",
            )
        else:
            ...
            if args.compare_output_json:
                semantic_check = _semantic_check_from_summary(
                    _load_json(args.compare_output_json),
                    result,
                )
        ...
        if args.role == "prefill":
            ...
            if result["worker_delta"].get("blocks_loaded", 0) != 0:
                raise SystemExit(
                    "Sequential handoff baseline failed: baseline unexpectedly loaded connector entries."
                )
            if result["worker_delta"].get("blocks_saved", 0) <= 0:
                raise SystemExit(
                    "Sequential handoff baseline failed: request did not save connector entries."
                )
            ...
            if semantic_check is not None and not semantic_check["match"]:
                raise SystemExit(
                    "Sequential handoff decode failed: semantic mismatch versus baseline "
                    f"({semantic_check['baseline_label']} vs {semantic_check['candidate_label']}, "
                    f"compare_by={semantic_check['compare_by']})."
                )
