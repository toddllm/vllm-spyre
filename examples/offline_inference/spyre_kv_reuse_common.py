...
def load_prompt_pair_spec(
    tokenizer,
    prompt_pair_json: str | Path,
) -> dict[str, Any]:
    spec_path = Path(prompt_pair_json).resolve()
    spec = json.loads(spec_path.read_text(encoding="utf-8"))

    prefill_prompt_text = spec.get("prefill_prompt_text")
    if not isinstance(prefill_prompt_text, str) or not prefill_prompt_text:
        raise ValueError(
            f"{spec_path} must define a non-empty 'prefill_prompt_text' field."
        )

    partial_prompt_text = spec.get("partial_prompt_text")
    if not isinstance(partial_prompt_text, str) or not partial_prompt_text:
        partial_prompt_text = prefill_prompt_text

    prefill_prompt_token_ids = tokenizer.encode(
        prefill_prompt_text,
        add_special_tokens=False,
    )
    partial_prompt_token_ids = tokenizer.encode(
        partial_prompt_text,
        add_special_tokens=False,
    )
    shared_prefix_tokens = common_prefix_len(
        prefill_prompt_token_ids,
        partial_prompt_token_ids,
    )

    display_name = (
        spec.get("display_name")
        or spec.get("question")
        or spec.get("trace_id")
        or spec_path.stem
    )

    return {
        "demo_template": spec.get("source_kind", "external_prompt_pair"),
        "demo_template_display_name": display_name,
        "scenario_text": spec.get("source_artifact") or spec.get("source_path"),
        "instruction_text": spec.get("question", ""),
        "partial_tail_text": spec.get("partial_tail_text", ""),
        "requested_shared_prefix_tokens": len(prefill_prompt_token_ids),
        "aligned_shared_prefix_tokens": shared_prefix_tokens,
        "prefill_prompt_token_ids": prefill_prompt_token_ids,
        "partial_prompt_token_ids": partial_prompt_token_ids,
        "partial_tail_tokens": max(
            0, len(partial_prompt_token_ids) - shared_prefix_tokens
        ),
        "exact_prompt_recompute_tokens": 0,
        "partial_prompt_recompute_tokens": max(
            0, len(partial_prompt_token_ids) - shared_prefix_tokens
        ),
        "prefill_prompt_text": prefill_prompt_text,
        "partial_prompt_text": partial_prompt_text,
        "prompt_pair_json": str(spec_path),
        "trace_id": spec.get("trace_id"),
        "question": spec.get("question"),
        "prompt_mode": spec.get("prompt_mode"),
    }
...
def get_worker_heap_kv_status() -> dict[str, Any]:
    connector = get_worker_connector()
    if connector is None:
        return {}

    status = getattr(connector, "get_heap_kv_status", None)
...
def extract_output_token_ids(outputs: list[Any]) -> list[int]:
    if not outputs:
        return []
