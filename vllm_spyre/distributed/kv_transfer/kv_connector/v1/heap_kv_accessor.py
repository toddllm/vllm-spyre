...
def get_layer_kv_tensor_info(perfdsc_dump_dir: str | Path) -> dict[str, dict[str, Any]]:
    ...
    node_tensor_prop_json = os.path.join(perfdsc_dump_dir, "sengraph", "nodeTensorProp_after_rcuOpt.json")
    ...
    with open(node_tensor_prop_json, "r") as f:
...
def get_heap_hbm_start_bytes(segment_size_json: str | Path, *, flit_size: int = 128) -> int:
    with open(segment_size_json, "r") as f:
