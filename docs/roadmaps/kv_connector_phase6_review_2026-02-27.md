# KV Connector Phase 6 Review (2026-02-27)

## Scope
Review and verification of Phase 6 work on branch `experimental/kv-connector-bridge-phase1` at commit `2fe2d95`, plus targeted correctness fixes.
...
Problem:
- In load path without explicit `block_mapping`, connector used `enumerate(req_meta.block_ids)` and treated the enumerate index as source block ID.
- This fails for non-zero source blocks (for example source block `10` was looked up as `0`).

Fix:
- In `_load_layer`, changed implicit mapping fallback from index-based to identity block-ID mapping:
...
- If source registry had fewer blocks than implied by matched external tokens, mapping was truncated.
- Connector would load only mapped subset without marking remaining external blocks invalid.

Fix:
- Added strict coverage check in `update_state_after_alloc`:
  - expected mapped blocks = `num_external_tokens // block_size`
  - if built mapping count differs, fallback to `is_store=True` request (recompute-safe path).
...
- `test_fallback_to_store_when_mapping_is_incomplete`
  - Verifies scheduler-side metadata generation falls back to store/recompute when external mapping cannot be fully constructed.
...
## Remote `toddllm` (CUDA env)
- `ruff check` on modified files: pass
- targeted regression tests: pass
- full worker suite: `148 passed`

## What the smaller agent still struggled with

1. **Block ID semantics under implicit mappings**
- It assumed positional mapping (`enumerate`) where block IDs are absolute scheduler IDs.
...
- It did not enforce that externally matched blocks are fully representable in metadata.
...
2. Decouple async layer-load worker env var from model-load process env var:
- `VLLM_SPYRE_MAX_LOAD_PROCESSES` currently carries two meanings.
- Introduce dedicated connector async env var for clarity and operational safety.

3. Add a targeted fault-injection test matrix:
